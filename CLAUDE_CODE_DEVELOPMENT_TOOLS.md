...
```bash
# コア設定フレームワーク
doom-emacs         # 高速でモダンなEmacs設定フレームワーク
                   # Evil-mode統合、LSP、Tree-sitter、パフォーマンス最適化済み
spacemacs          # コミュニティ駆動の階層型設定フレームワーク
                   # VimとEmacsのキーバインド統一、豊富なレイヤー機能
use-package        # 宣言的パッケージ管理システム
                   # 遅延読み込み、依存関係管理、設定の構造化
